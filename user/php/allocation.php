...
// Fetch barangays
$barangayQuery = "SELECT barangay_id, barangay FROM barangay";
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $activity_type = 'allocation'; // Fixed activity type for allocation
    $barangay_id = intval($_POST['barangay_id']);
    $description = $_POST['medicine_id']; // Use the selected medicine description from the dropdown
    $quantity = intval($_POST['quantity']);
    $activity_date = $_POST['activity_date'] ?? $activity_date; // Use submitted date or default to today

    // Validate input
    if (empty($barangay_id) || empty($description) || $quantity <= 0) {
        ...
        if ($quantity > $totalStock) {
            ...
            // Deduct stock from each medicine until the requested quantity is allocated
            foreach ($medicineStocks as $stock) {
                if ($remainingQuantity <= 0) {
                    break; // Exit if no remaining quantity to allocate
                }

                // Calculate how much to deduct from the current stock
                $deductQuantity = min($stock['stock_quantity'], $remainingQuantity);

                // Check to ensure we don't deduct more than available stock
                if ($deductQuantity > 0) {
                    ...
                    $remainingQuantity -= $deductQuantity; // Decrease the remaining quantity needed
                }
            }

            // Insert into allocation table after stock deduction
            $insertAllocationQuery = "
                INSERT INTO allocation (barangay_id, medicine_id, quantity, activity_date)
                VALUES (?, ?, ?, ?)
            ";

            // Insert an entry for each medicine deducted
            foreach ($medicineStocks as $stock) {
                if ($quantity <= 0) break; // No more quantity to allocate

                $allocatedQuantity = min($stock['stock_quantity'], $quantity);
                
                if ($allocatedQuantity > 0) {
                    $stmt = $conn->prepare($insertAllocationQuery);
                    $stmt->bind_param("iiis", $barangay_id, $stock['medicine_id'], $allocatedQuantity, $activity_date);
                    $stmt->execute();
                    $quantity -= $allocatedQuantity; // Reduce the quantity to allocate
                }
            }
...
<div class="main-content">
    <h1 class="form-title">Medicine Allocation</h1>
